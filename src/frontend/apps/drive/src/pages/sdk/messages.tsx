...
export default function MessagesPicker() {
  ...
  const config = useQuery({
    queryKey: ["messages-picker-config"],
    queryFn: () => storageRequest<{ MESSAGES_PUBLIC_URL: string }>("config/"),
  });
  ...
  const listing = useQuery({
    queryKey: ["messages-picker", current?.id, current?.space, offset],
    queryFn: async () => {
      if (current) {
  ...
  const allowed = Boolean(config.data?.MESSAGES_PUBLIC_URL && router.query.origin === new URL(config.data.MESSAGES_PUBLIC_URL).origin);
  const choose = (action: "folder" | "link" | "copy") => {
    ...
    }, new URL(config.data!.MESSAGES_PUBLIC_URL).origin);
    window.close();
  };
  if (config.isPending) return <p role="status">{t("storage.loading")}</p>;
...
MessagesPicker.getLayout = (page: React.ReactElement) => getSdkLayout(<ColumnPreferencesProvider>{page}</ColumnPreferencesProvider>);
