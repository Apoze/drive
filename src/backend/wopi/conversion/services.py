...
def _validate_conversion(item, user, require_ready=True):
    ...
    if not client_options.get("ConvertServiceUrl"):
...
def resolve_backend(client_options):
    ...
    return OnlyOfficeConversionBackend(convert_service_url=convert_service_url)


def _resolve_destination_parent(source_item, user):
    """Return the source parent when the user can still create there, else None."""
...
def prepare_conversion(source_item, user):
    """Create a placeholder item receiving the future conversion result."""
    target_extension, _client_options = _validate_conversion(
        source_item,
        user,
        require_ready=False,
    )
...
def perform_conversion(source_item, placeholder, user):
    """Run OnlyOffice conversion and attach the result to the placeholder."""
