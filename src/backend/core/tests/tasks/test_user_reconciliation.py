...
def test_user_reconciliation_csv_import_invalid_email():
    ...
    assert csv_import.status == "done"
    assert "Invalid inactive email address in CSV row" in csv_import.logs
    assert "active1@example.com" not in csv_import.logs
    assert "40" not in csv_import.logs
    assert models.UserReconciliation.objects.count() == 0
...
def test_user_reconciliation_csv_import_same_email_redacts_log():
    """Same-email validation logs row context without raw email addresses."""
    factories.UserFactory(email="active1@example.com")
    csv_import = models.UserReconciliationCsvImport.objects.create(
        file=ContentFile(
            b"active_email,inactive_email,id\nactive1@example.com,active1@example.com,42\n",
            name="same-email.csv",
        )
    )
    ...
    assert csv_import.status == "done"
    assert "Same address set as both active and inactive email" in csv_import.logs
    assert "active1@example.com" not in csv_import.logs
    assert "42" not in csv_import.logs


def test_user_reconciliation_csv_import_errors_do_not_store_tracebacks():
    """Import failures keep admin-visible logs bounded and path-free."""
    csv_import = models.UserReconciliationCsvImport.objects.create(
        file=ContentFile(b"", name="empty.csv")
    )
    ...
    assert csv_import.status == "error"
    assert "missing mandatory columns" in csv_import.logs
    assert "Traceback" not in csv_import.logs
