...
@override_settings(METRICS_ENABLED=True)
def test_usage_metrics_list_filter_by_legacy_account_id(api_key):
    """The former account_id=<sub> filter remains supported for compatibility."""
    client = APIClient()
    user1 = factories.UserFactory()
    factories.UserFactory()
...
@override_settings(METRICS_ENABLED=True, METRICS_USER_CLAIMS_EXPOSED=["siret"])
...
@override_settings(METRICS_ENABLED=True, METRICS_USER_CLAIMS_EXPOSED=["siret"])
def test_usage_metrics_organization_type_invalid_claim_key(api_key):
    """Organization metrics reject claim keys outside the exposed allowlist."""
    client = APIClient()

    response = client.get(
        "/external_api/v1.0/metrics/usage/"
        "?account_type=organization&account_id_key=department"
        "&account_id_value=hr",
        HTTP_AUTHORIZATION=f"Api-Key {api_key}",
    )
    assert response.status_code == 400
