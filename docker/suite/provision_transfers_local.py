...
with transaction.atomic():
 ...
 service=Service.objects.filter(config__suite_app_id='transfers').first()
 if service is None:
  service=Service.objects.create(type='transfers',name='Transfers',instance_name='Suite locale',url=p['origin'],is_active=True,hidden=True,
   config={'suite_app_id':'transfers','allowed_organization_ids':[org],'entitlements_api_key':p['policy_key']})
 ...
 for scope in ('organization','user'):
  Entitlement.objects.get_or_create(service_subscription=subscription,type='transfers_storage',account_type=scope,account=None,defaults={'config':{'max_storage':20_000_000_000}})
...
def provision(state, suite, repo, *, keycloak_adapter=False):
    ...
    config.update(shell('st-deploycenter-backend-dev-1', ST, config, state / 'st-registration.log'))
