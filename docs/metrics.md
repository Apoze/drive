...
- `account_id_key` | The key to filter on (`sub` or `email` for `account_type=user`). Required if `account_type=organization`; the key must be listed in `METRICS_USER_CLAIMS_EXPOSED`.
- `account_id_value` | The value to filter on. Required if `account_type=organization`.
- `account_id` | Legacy compatibility alias for `account_id_key=sub&account_id_value=<sub value>` when `account_type=user`.
...
GET /external_api/v1.0/metrics/usage/?account_type=user&account_id=<sub value>
...
> You should mind to add `siret` into both `OIDC_STORE_CLAIMS` and
> `METRICS_USER_CLAIMS_EXPOSED` so the siret is stored and explicitly allowed
